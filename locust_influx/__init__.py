...
def __listen_for_requests_events(node_id, measurement: str = 'locust_requests') -> Callable:
    ...
    def event_handler(request_type=None, name=None, response=None, response_time=None, response_length=None, exception=None,
                      **_) -> None:
        time = datetime.utcnow()
        tags = {
            'node_id': node_id,
...
def expose_metrics(influx_host: str = 'localhost',
    ...
    # events.request.add_listener(__listen_for_requests_events(node_id, success=False))
    # Locust events
    # events.request.add_listener(__listen_for_locust_events(node_id, event='hatch_complete'))
    # events.request.add_listener(__listen_for_locust_events(node_id, event='quitting'))
    # events.request.add_listener(__listen_for_locust_events(node_id, event='master_start_hatching'))
    # events.request.add_listener(__listen_for_locust_events(node_id, event='master_stop_hatching'))
    # events.request.add_listener(__listen_for_locust_events(node_id, event='locust_start_hatching'))
    # events.request.add_listener(__listen_for_locust_events(node_id, event='locust_stop_hatching'))
    # # Locust exceptions
    # events.request.add_listener(__listen_for_locust_errors(node_id))
    ...
    # events.quitting.add_listener(last_flush_on_quitting)
